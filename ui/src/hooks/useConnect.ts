...
const L2_METADATA = {
  ...
  description: 'BOB Unified Asset Tracker Demo',
...
const getClientsAndAccounts = async () => {
  ...
  const walletClient = createWalletClient({ chain: l2chain, transport: custom(window.ethereum) });
...
const useConnect = () => {
  ...
  useEffect(() => {
    if (connected) {
      (async () => {
        const { publicClient, walletClient, evmAddress, bitcoinAddress, xpub } = await getClientsAndAccounts();
  ...
  return { connect, publicClient, walletClient, evmAccount, bitcoinAddress, xpub };
};

export { L2_CHAIN_CONFIG, L2_METADATA, L2_PROJECT_ID, useConnect };
