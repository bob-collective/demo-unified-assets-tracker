...
pub async fn get_value_in_usd(symbol_to_find: &str) -> String {
    ...
    if let Some(coin) = find_id_by_symbol(&json_data, symbol_to_find) {
        let url = "https://api.coingecko.com/api/v3/simple/price?ids=".to_string() + &coin.name + "&vs_currencies=usd";
        let response = reqwest::get(url).await.unwrap();
        if let Ok(json_result) = response.json::<serde_json::Value>().await {
            if json_result == String::from("null") {
                return String::new();
            }

            return json_result[coin.name]["usd"].to_string();
